...
class StoryResponseReviewTests(TestCase):
    def setUp(self):
        ...
        self.section = Section.objects.create(school=self.school, class_code="STORY", class_name="Story", teacher=self.teacher)
        ...
        self.material = Material.objects.create(teacher=self.teacher, section=self.section, title="Tell me", item_type="paragraph", type="assessment", status="published", content_json={"activity_type": "story_response", "source_story_reading_material_id": self.story.id, "response_prompt": "What happened?"})
