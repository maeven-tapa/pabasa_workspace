...
from .azure_stt import AzureSpeechError, transcribe_azure_audio
...
@override_settings(AZURE_SPEECH_KEY='private-test-key', AZURE_SPEECH_REGION='southeastasia')
class AzureSpeechAdapterTests(SimpleTestCase):
    @patch('pabasa_app.azure_stt.requests.post')
    def test_browser_formats_and_locales_are_sent_to_azure(self, post):
        post.return_value = Mock(status_code=200)
        post.return_value.json.return_value = {'combinedPhrases': [{'text': 'Si Bibo'}, {'text': 'ay bata.'}]}
        for mime in ('audio/webm;codecs=opus', 'audio/ogg', 'audio/mp4', 'audio/wav'):
            for locale in ('fil-PH', 'en-PH'):
                with self.subTest(mime=mime, locale=locale):
                    self.assertEqual(transcribe_azure_audio(recording(mime=mime), locale),
                                     ('Si Bibo ay bata.', 'azure_fast', ''))
                    args, kwargs = post.call_args
                    self.assertEqual(args[0], 'https://southeastasia.api.cognitive.microsoft.com/'
                                             'speechtotext/transcriptions:transcribe?api-version=2025-10-15')
                    self.assertEqual(kwargs['headers']['Ocp-Apim-Subscription-Key'], 'private-test-key')
                    self.assertEqual(json.loads(kwargs['data']['definition']), {'locales': [locale]})
                    self.assertEqual(kwargs['files']['audio'][1:], (b'browser audio', mime))
                    self.assertEqual(kwargs['timeout'], (5, 12))
                    self.assertFalse(kwargs['allow_redirects'])

    @patch('pabasa_app.azure_stt.requests.post')
    def test_configuration_and_clip_validation_prevent_network_requests(self, post):
        for key, region in (('', 'southeastasia'), ('key', ''), ('key', 'invalid/region')):
            with self.subTest(key=bool(key), region=region), override_settings(AZURE_SPEECH_KEY=key, AZURE_SPEECH_REGION=region):
                with self.assertRaises(AzureSpeechError) as error:
                    transcribe_azure_audio(recording(), 'fil-PH')
                self.assertEqual(error.exception.status, 503)
        for audio, status in ((recording(b''), 400), (Mock(size=12 * 1024 * 1024 + 1), 413)):
            with self.assertRaises(AzureSpeechError) as error:
                transcribe_azure_audio(audio, 'fil-PH')
    ...
    def test_errors_do_not_disclose_provider_details(self, post):
        for status in (302, 400, 401, 403, 429, 500):
            ...
            with self.subTest(status=status), self.assertRaises(AzureSpeechError) as error:
                transcribe_azure_audio(recording(), 'fil-PH')
    ...
    def test_silence_and_invalid_responses(self, post):
        post.return_value = Mock(status_code=200)
        post.return_value.json.return_value = {'combinedPhrases': []}
        self.assertEqual(transcribe_azure_audio(recording(), 'fil-PH')[0], '')
        for data in ({}, None, {'combinedPhrases': None}, {'combinedPhrases': [None]}, {'combinedPhrases': [{'text': 12}]}):
            post.return_value.json.return_value = data
            with self.subTest(data=data), self.assertRaises(AzureSpeechError):
                transcribe_azure_audio(recording(), 'fil-PH')
        post.return_value.json.side_effect = ValueError('invalid JSON')
        with self.assertRaises(AzureSpeechError):
            transcribe_azure_audio(recording(), 'fil-PH')


@override_settings(GOOGLE_STT_API_KEY='google-test-key', AZURE_SPEECH_KEY='', AZURE_SPEECH_REGION='')
class AzureSpeechRoutingTests(SimpleTestCase):
    def setUp(self):
    ...
    @patch('pabasa_app.views.transcribe_azure_audio', return_value=('bata', 'azure_fast', ''))
    def test_azure_bypasses_google_configuration_and_keeps_grading(self, azure, google):
        for language, locale in (('Filipino', 'fil-PH'), ('English', 'en-PH')):
            response = reading_transcribe_api(self.request('azure', language))
            self.assertEqual(response.status_code, 200)
            result = json.loads(response.content)
            self.assertTrue(result['complete'])
            self.assertEqual(result['stt_provider'], 'azure')
            self.assertEqual(result['stt_model'], 'azure_fast')
            self.assertEqual(azure.call_args.args[1], locale)
        google.assert_not_called()

    @patch('pabasa_app.views.transcribe_azure_audio')
    @patch('pabasa_app.views.transcribe_audio_bytes_with_model', return_value=('bata', 'stt_v1', ''))
    def test_google_is_default_and_restored_when_off(self, google, azure):
    ...
    @patch('pabasa_app.views.transcribe_audio_bytes_with_model')
    def test_missing_azure_configuration_does_not_fall_back(self, google):
        response = reading_transcribe_api(self.request('azure'))
        self.assertEqual(response.status_code, 503)
        self.assertIn('Microsoft Azure', json.loads(response.content)['error'])
        google.assert_not_called()

    @patch('pabasa_app.views._lesson1_ffmpeg_binary')
    @patch('pabasa_app.views.transcribe_azure_audio', return_value=('A B K', 'azure_fast', ''))
    def test_lesson_one_uses_azure_without_ffmpeg(self, azure, ffmpeg):
    ...
    @patch('pabasa_app.views.transcribe_azure_audio')
    def test_authentication_and_student_access_still_apply(self, azure):
