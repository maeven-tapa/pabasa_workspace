...
def _signup_section_queryset(role, school=None, grade_level=None):
    ...
    if role == 'teacher':
        # Only an active teacher account occupies a Section. Legacy rows that
        # point to archived/non-teacher users remain visible for normalization.
        qs = qs.exclude(teacher__role='teacher', teacher__is_archived=False)
    return qs


def _section_signup_payload(section):
    ...
    return {
        ...
        # The queryset applies role-specific filtering; keep availability
        # explicit so the template does not treat a missing value as false.
        'is_available': bool(section.is_active),
        'has_teacher': section.has_active_teacher(),
        'teacher_name': f"{teacher.first_name} {teacher.last_name}".strip() if teacher else '',
        'student_count': section.get_student_count(),
    }


def _section_selection_context(role):
    grade_map = {grade: [] for grade in SCHOOL_GRADE_LEVELS}
    available_grades = set()
    for section in _signup_section_queryset(role):
        if section.grade_level in grade_map:
            available_grades.add(section.grade_level)
            grade_map.setdefault(section.grade_level, []).append(_section_signup_payload(section))
    signup_grades = [
        {'label': grade, 'value': grade, 'is_available': grade in available_grades}
...
def _official_crla_materials_for_phase(assessment_type):
    ...
    return _official_reading_active_materials_queryset().filter(
        ...
        Q(system_assessment_phase=assessment_type) |
...
def _store_pending_teacher_signup(request, data):
    ...
    _set_pending_data(request, 'pending_teacher_signup', {
        ...
        # Keep the legacy field available for existing accounts, but do not
        ...
        if active_school_calendar:
            ...
            student_calendar_events = [
                _calendar_event_payload(event) | {
                    ...
                    **({'scope': event.scope} if nav_role in {'teacher', 'student'} else {}),
                }
...
def _admin_user_template_context(request, user, page_title):
    ...
    context.update({
        ...
        'current_section': getattr(current_section, 'section', None) if user.role == 'student' and current_section else current_section,
        # Keep shared edit-template iterables defined on every render path.
        'school_grades': [],
        'available_sections': [],
...
def _admin_edit_user(request, user_id, role):
    ...
    if request.method == 'POST':
        ...
        elif destination_grade and destination_section_value:
            destination_section = _active_canonical_section(destination_grade, destination_section_value)

        if role == 'teacher' and (destination_grade or destination_section_value):
            if destination_grade and destination_grade not in SCHOOL_GRADE_LEVELS:
            ...
            with transaction.atomic():
                ...
                if destination_section.teacher_id and destination_section.teacher_id != user.id:
                    replace = str(request.POST.get('replace_teacher') or '').strip().lower() in {'1', 'true', 'yes', 'on'}
                    if not replace:
                        context = _admin_user_template_context(request, user, 'Edit Teacher')
                        context['warning_message'] = (
                            f"{destination_section.grade_level} - {destination_section.section} is already assigned to "
                            f"{destination_section.active_teacher_name() or 'another teacher'}."
                        )
                ...
                destination_section.teacher = user
                destination_section.save(update_fields=['teacher', 'updated_at'])

        if role == 'student' and (destination_grade or destination_section_value):
            if destination_grade and destination_grade not in SCHOOL_GRADE_LEVELS:
...
def _official_reading_active_materials_queryset():
    return _official_reading_materials_queryset().filter(is_active=True)
...
def _official_assessment_edit_context(request, material=None):
    ...
    return {
        ...
        'is_read_only': bool(material and getattr(material, 'pk', None)),
    }


def _save_official_reading_assessment(request, material=None):
    field_errors = {}
    calendar_id_raw = request.POST.get('calendar_id')
    ...
    try:
        ...
        calendar_id = None
    if calendar_id:
        selected_calendar = SchoolCalendar.objects.filter(pk=calendar_id).first()
        if selected_calendar:
            school_year_value = str(getattr(selected_calendar, 'school_year', '') or '').strip()
    if not school_year_value:
        _, calendars, active_calendar = _selected_school_calendar(request)
        school_year_value = str(getattr(active_calendar, 'school_year', '') or '').strip()
        if not school_year_value and calendars:
            school_year_value = str(getattr(calendars[0], 'school_year', '') or '').strip()
    try:
    ...
    with transaction.atomic():
        if material and getattr(material, 'pk', None):
            ...
            target = Material()
        passage_texts = [str(p.get('text') or p.get('content') or '').strip() for p in passages if isinstance(p, dict)]
        target.is_official_reading = True
        target.is_system_owned = bool(getattr(target, 'is_system_owned', False))
...
def _toggle_official_material_state(material, activate):
...
def admin_official_reading_assessment_add(request):
    if request.method == 'POST':
        ...
        if error_response:
            return render(request, 'pabasa_app/admin_official_reading_assessment_add.html', dict(_official_assessment_edit_context(request, None), **{
        ...
        return redirect(reverse('admin_official_reading_assessment_detail', args=[saved_material.id]))
    return render(request, 'pabasa_app/admin_official_reading_assessment_add.html', _official_assessment_edit_context(request, None))
...
def admin_official_reading_assessments(request):
    if request.method == 'POST':
        ...
        if not material or not material.is_official_reading:
            return HttpResponseForbidden("Unable to update official assessment.")
        if action == 'archive':
            _toggle_official_material_state(material, False)
        elif action == 'unarchive':
            _toggle_official_material_state(material, True)
        else:
            return HttpResponseBadRequest("Unsupported action.")
        return redirect(reverse('admin_official_reading_assessments'))
...
def _official_assessment_detail_context(request, material):
    ...
    return {
        ...
        'last_name': request.session.get('last_name', ''),
        'material': material,
        'selected_assessment_type': _official_reading_assessment_type(material),
        'school_year_value': getattr(active_calendar, 'school_year', '') or '',
        'current_term_label': f"Term {getattr(active_calendar, 'current_term', '')}" if active_calendar else 'No Active Term',
        'selected_calendar': selected_calendar,
        'school_calendars': calendars,
        'current_school_calendar': active_calendar,
...
def _get_official_reading_material(material_id):
    return Material.objects.filter(pk=material_id, is_official_reading=True).first()


@admin_required
@require_http_methods(["POST"])
def admin_official_reading_assessment_archive(request, material_id):
    material = _get_official_reading_material(material_id)
    if not material:
        return redirect('admin_official_reading_assessments')
    _toggle_official_material_state(material, False)
...
@admin_required
@require_http_methods(["POST"])
def admin_official_reading_assessment_unarchive(request, material_id):
    material = _get_official_reading_material(material_id)
    if not material:
        return redirect('admin_official_reading_assessments')
    _toggle_official_material_state(material, True)
...
def _section_course_payload(section):
    ...
    assessments_qs = Assessment.objects.filter(
        teacher=section.teacher, section=section,
        source_assessment__isnull=True, is_active=True,
    ...
    assessments = [{
        'id': assessment.id, 'raw_id': assessment.id,
        'action_id': f'assessment-{assessment.id}', 'code': assessment.code,
        'title': assessment.title, 'assessment_type': assessment.assessment_type,
        'level': assessment.get_assessment_type_display(), 'status': assessment.status,
    ...
    for material in materials_qs:
        ...
        material_payload = {
            ...
            'assigned_sections': [section.class_code], 'assigned_week': material.assigned_week,
            'assigned_week_display': format_assigned_week_display(material.assigned_week),
    ...
    practices = [{
        ...
        'action_id': f'practice-{practice.id}', 'record_kind': 'practice',
        'title': practice.title, 'status': practice.status,
    ...
    return {
        ...
        'sections': [{'id': section.id, 'code': section.class_code, 'name': section.class_name}],
        ...
        for c in courses_qs:
            ...
            for m in c.materials.all():
                ...
                materials_list.append({
                    ...
                    'template_title': content_json.get('template_title') or '',
                    'template_lesson': content_json.get('template_lesson') or '',
                    'template_type': content_json.get('template_type') or content_json.get('template_title') or '',
                    'activity_type': content_json.get('activity_type') or '',
                    'activity_format': content_json.get('activity_format') or '',
                    'activity_id': content_json.get('activity_id') or '',
                    'selected_set_id': content_json.get('activity_id') or '',
        ...
        if course_id is not None:
            ...
            if course_id_text.startswith('section-'):
                ...
                class_section = Section.objects.filter(
                    id=section_id, teacher=teacher_user, is_active=True,
                ).first()
        ...
        try:
            ...
            if class_section is not None:
                materials_qs = Material.objects.filter(
                    ...
                    teacher=teacher_user,
                    type__in=['assessment', 'both'], is_active=True,
                ).distinct()
