(() => {
  ...
  function render(){
    ...
    document.getElementById('confirm-restart').onclick=async()=>{if(busy)return;busy=true;try{await send({action:'restart'});document.getElementById('confirm').hidden=true;render();}catch(e){const message=e.message||'Hindi na-save ang gawain.';setFeedback(message,true);await speak(message);}finally{busy=false;}};
    ...
    const finish=async e=>{if(phase!=='SELECTING'||!active||e.pointerId!==active.id)return;const gesture=active;active=null;paint([]);phase='VALIDATING';try{const cell=document.elementFromPoint(e.clientX,e.clientY)?.closest('.cell');const path=cell?pathBetween(gesture.start,[+cell.dataset.row,+cell.dataset.col]):gesture.path;if(path.length<2){phase='READY';return;}busy=true;const word=words.find(w=>JSON.stringify(paths[w])===JSON.stringify(path))||'';const result=await send({action:'select_word',word,path,color:'#b6e6c3'});const message=result.state.last_feedback||'';render();setFeedback(message);await speak(message);}catch(err){const message=err.message||'Hindi na-save ang gawain.';setFeedback(message,true);await speak(message);}finally{busy=false;if(!state.completed)phase='READY';}};
