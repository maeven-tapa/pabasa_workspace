...
(() => {
  ...
  async function startCReading(){
    ...
    try{
      ...
      const form=new FormData();form.append('audio',audio,'reading.webm');message('Sinusuri ang pagbasa...');
      await send({action:'reading_syllable_attempt'},form,false);render();
      message(state.read_aloud_completed?'Magaling! Nabasa mo nang tama ang lahat ng pantig.':state.last_feedback==='Tama!'?'Tama!':'Subukan muli.');
    }catch(error){
