(() => {
  ...
  async function record(item) { if(busy || isPaused || isMuted) { if (isMuted) publishDebug({status:'Muted', error:'—'}, 'Recording blocked while muted'); return; } const attempt=++speechGeneration; speechAttemptActive=true; const button=document.getElementById('read'); if(!navigator.mediaDevices?.getUserMedia||!window.MediaRecorder){render('Microphone recording is not available in this browser.','bad');speechAttemptActive=false;return;} busy=true; button?.classList.add('is-busy'); publishDebug({status:'Listening', expected:item.word, error:'—'}, 'Recording started'); try { const result = await window.Basahin.read({target_text:item.word, language:'English', mode:'reading'}, {button:button, url:data.transcribe_url}); if(attempt!==speechGeneration)return;const transcript=String(result.raw_transcript||result.transcript||'');const normalized=normalize(transcript);publishDebug({status:'Processing', transcript, normalized, expected:item.word}, 'Transcription received');const saved=await post(data.progress_url,{action:'oral_read',item_index:state.current_item,success:result.complete===true});if(attempt!==speechGeneration)return;const correct=saved.progress?.state?.phase==='letter';busy=false;publishDebug({status:correct?'Correct':'Try again', result:correct?'Match':'Not Match', transcript, normalized, expected:item.word, error:'—'}, correct?'Match':'Retry');render(correct?'Correct! Now write the missing letter.':`I heard “${transcript}”. Try again.`,correct?'good':'bad');if(attempt===speechGeneration)await playAudio(correct?READ_CORRECT_FEEDBACK:RETRY_FEEDBACK); } catch(error){ if (error?.name === 'AbortError') return; if(attempt===speechGeneration){stopStream();busy=false;publishDebug({status:'Error', error:error.message||'I could not hear you.'}, 'Error: '+(error.message||'recording failed'));render(error.message||'I could not hear you. Try again.','bad');}} finally{if(attempt===speechGeneration){speechAttemptActive=false;if(button?.isConnected)button.classList.remove('is-busy');busy=false;publishDebug({status:isPaused?'Paused':'Ready', recorder:'inactive'});}} }
