...
(function () {
    ...
    function recoveryModal(draft, onResume, onDiscard) {
        ...
        root.className = 'modal'; root.style.cssText = 'display:flex;position:fixed;inset:0;z-index:25000;align-items:center;justify-content:center;padding:1rem;background:rgba(15,23,42,.55)';
        root.innerHTML = `<div class="modal-dialog" style="position:relative;z-index:25001;width:min(100%,460px);margin:0"><div class="modal-content"><div class="modal-header"><h5 class="modal-title">Live CRLA Session Interrupted</h5></div><div class="modal-body"><p>An unfinished Live CRLA assessment was found for Batch ${Number(draft.batchNumber) || 1}. The last recorded temporary progress is available for recovery.</p><p class="mb-0 text-muted">Resume the assessment to return to the Student Monitor with the batch, students, progress, and assessment time restored.</p></div><div class="modal-footer"><button class="btn btn-outline-danger" data-discard>Discard Session</button><button class="btn btn-primary" data-resume>Resume Assessment</button></div></div></div>`;
