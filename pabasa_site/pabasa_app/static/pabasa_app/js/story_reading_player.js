...
(() => {
    ...
    function tick(now) { if (!state.playing) return; if (!lastFrameTime) lastFrameTime = now; state.time += Math.min(.1, (now - lastFrameTime) / 1000); lastFrameTime = now; if (state.time >= totalDuration) { state.time = totalDuration; state.playing = false; state.completed = true; finishCompletion(); } render(); frame = requestAnimationFrame(tick); }
