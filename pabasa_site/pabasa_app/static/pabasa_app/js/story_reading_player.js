...
(() => {
    ...
    const restoreState = () => {
        ...
        // The server record is authoritative for this student's material.
        // A completed activity is view-only and reopens its saved result.
        if (server.completed) {
            state.time = totalDuration;
            state.scene = scenes.length;
            state.completed = true;
            state.correctSentences = Math.max(0, Math.min(totalSentences, Number(server.correct_sentences) || 0));
            state.readingScore = state.correctSentences;
            return;
        }
        // A stale client-side completed flag must never complete a new session.
        if (saved?.completed) {
    ...
    function seek(value) { if (state.completed) return; stopTts(); state.time = Math.min(totalDuration, Math.max(0, Number(value) || 0)); state.completed = false; state.readingCursor = 0; render(); persist(); }
