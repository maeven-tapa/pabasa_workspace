(() => {
  ...
  if (![8, 9].includes(Number(activity.session)) || data.preview) return;
  const main = document.querySelector('main');
  if (!main || document.querySelector('.prescribed-intro-overlay')) return;
  const esc = value => String(value ?? '').replace(/[&<>"']/g, c => ({'&':'&amp;','<':'&lt;','>':'&gt;','"':'&quot;',"'":'&#39;'}[c]));
  const lesson = activity.lesson ? `LESSON ${activity.lesson}` : '';
  const gawain = activity.activity_number ? `GAWAIN ${activity.activity_number}` : '';
  ...
  const narrate = async () => {
    ...
    form.append('target_text', activity.instruction);
    form.append('language', activity.language || 'Filipino');
    ...
    try {
      ...
      if (!response.ok || !result.success || !result.audio_content) return;
      audio = new Audio(`data:${result.mime_type || 'audio/mpeg'};base64,${result.audio_content}`);
      audio.onended = () => { audio = null; };
      await audio.play().catch(() => {});
    } catch (_) { /* Autoplay/TTS failure must not block the learner. */ }
  };
